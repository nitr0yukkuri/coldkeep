...
Expo経路の推論エンジン表示はTypeScriptになります。通常モードでは、振り音の残量モデルと
氷量モデルは未学習のまま`未判定`を表示します。研究・デモでのみ、下記の明示的なフラグを
使って低信頼度の実験推論を確認できます。実験値は飲水量の自動記録には使いません。

研究専用の氷量パイプラインまで画面で確認したい場合だけ、Metro起動前に次を設定します。
...
このモードは外部音源由来の149次元研究artifactをTypeScriptで実行し、氷量を
`experimental`として表示します。通常ビルドでは無効で、実測ColdKeepモデルの代わりには
なりません。研究プレビューの値は自動飲水記録へ流れず、精度の証拠として扱えません。
終了時は環境変数を削除してMetroを再起動してください。
