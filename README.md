...
水判定または充填判定の確率が0.65未満の場合は、画面を `未判定` に戻して
...
氷の重量、正確な氷個数、水温、保冷時間は推定しません。氷の有無も、学習済みモデルの信頼度が0.65以上の場合だけ表示します。未検証の条件では結果を強制せず、エラー時も `未判定` に戻します。
...
個人向けの画面では、水筒容量と1日の目標量を設定し、飲んだ量を手動で記録できます。
音響スキャンの充填結果は端末内に残量観測として保存され、前回との差分を確認してから
参考値として飲水量へ追加できます。個人向けの音響チェックも、現行モデルの学習条件に合わせて
水筒へ水を注ぐ音を入力します。振る音や飲む音を直接測定する機能ではありません。
...
研究用ベースラインと個人向け画面は、`pour`（注ぐ）を入力してください。`shake`（振る）は
別モデルのデータ収集用で、現行スキャンの入力にはしません。`still`（静置）も比較データを
集めるためのラベルです。
...
詳細は [DATA_COLLECTION.md](DATA_COLLECTION.md) を参照してください。氷モデルは `ice_count > 0` の二値分類で、正例・負例の両方が揃うまで学習を開始しません。スキャン後は、未学習なら氷の有無を `未判定` と表示します。
