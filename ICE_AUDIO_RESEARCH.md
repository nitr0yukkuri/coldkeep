...
#### Laser Vibrations metadata audit

The strongest count-adjacent lead was audited from the public
[Laser Vibrations metadata snapshot](https://huggingface.co/datasets/eturok-weizmann/laser-vibrations).
The reproducible, metadata-only result is
[`ml/reports/laser_vibrations_count_audit.json`](ml/reports/laser_vibrations_count_audit.json),
generated by [`ml/audit_count_adjacent_metadata.py`](ml/audit_count_adjacent_metadata.py).
The 551 valid rows contain exactly 4 `n_objects=0`, 527 `n_objects=1`, and 20
`n_objects=2` samples. All rows are cardboard-box experiments with `cube` or
`empty` objects and four coded speaker configurations; experiment IDs and the
reconstructed-audio URLs are unique. The dataset card describes laser-speckle
vibration reconstruction and loudspeaker excitation, not phone-microphone
recordings or handheld bottle shaking. The metadata exposes no `3+` class and
the raw repository is very large, so downloading it wholesale would not improve
the ColdKeep evidence.

The audit intentionally records `productionLabelEligible=false`, never maps
`n_objects` to `none`/`few`/`many`, and does not import a row into the ColdKeep
manifest. It is suitable only for auxiliary count-regression experiments,
transient-feature design, or a domain-gap sanity check. The default audio URL
in the source metadata is a shared chirp stimulus; the audit instead uses the
per-sample reconstructed-audio URL to avoid mistaking the common excitation for
