...
スマートフォンのマイクで水筒を軽く叩いた音を録音し、端末内の音響特徴量と分類モデルから、水の有無と粗い充填クラス（50% / 90%）を表示します。氷のラベルが未学習の間は、氷を `未判定` と表示します。
...
現行モデルの評価はACM-S2の小規模なコンテナ分離評価です。水の有無18/18、充填5/6という結果がありますが、未知の水筒・端末への精度保証ではありません。周囲の音、水筒材質、距離、叩き方で結果が変わります。
